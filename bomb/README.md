...
- [x] `phase_1`
  ...
  查看内存位置`x/s $0x402400`后得到`Border relations with Canada have never been better.`

- [x] `phase_2`

  调用`read_six_number`，传入`%rsp`作为内存地址

  依次检查返回值，要求`x[0]==1`,`x[i+1]==x[i]*2`

- [x] `phase_3`

  在行中读取两个数`x1`,`x2`

  先检测`x1<7`，然后进入一个类似`switch`的结构
...
- [x] `phase_4`
  ...
  然后调用`func4(x1,0,14)`，不是很懂这个函数是干啥的，一顿分析完给出一个可行解就结束了...

- [x] `phase_5`

  读入6个字符

  根据每个字符ASCII的低8位`k`，在一个预定义的字符串里取`str[k]`
...
- [x] `special_phase`
  ...
  需要在第4行传入两个整数和`DrEvil`

  之后调用`fun7`，这个函数类似于链表中找元素，也是一顿分析...
...
0000000000400ee0 <phase_1>:
  ...
  400ee4:       be 00 24 40 00          mov    $0x402400,%esi	# preload string
...
0000000000400efc <phase_2>:
  ...
  400f0a:       83 3c 24 01             cmpl   $0x1,(%rsp)	# x[0]=1
  400f0e:       74 20                   je     400f30 <phase_2+0x34>
  400f10:       e8 25 05 00 00          callq  40143a <explode_bomb>
  400f15:       eb 19                   jmp    400f30 <phase_2+0x34>
  400f17:       8b 43 fc                mov    -0x4(%rbx),%eax	# x[i+1]=2*x[i]
...
0000000000400f43 <phase_3>:
  ...
  400f47:       48 8d 4c 24 0c          lea    0xc(%rsp),%rcx	# x2
  400f4c:       48 8d 54 24 08          lea    0x8(%rsp),%rdx	# x1
  400f51:       be cf 25 40 00          mov    $0x4025cf,%esi	# %d %d
  400f56:       b8 00 00 00 00          mov    $0x0,%eax
  400f5b:       e8 90 fc ff ff          callq  400bf0 <__isoc99_sscanf@plt>	# read 2 int
  400f60:       83 f8 01                cmp    $0x1,%eax
  400f63:       7f 05                   jg     400f6a <phase_3+0x27>	# read finish
  400f65:       e8 d0 04 00 00          callq  40143a <explode_bomb>
  400f6a:       83 7c 24 08 07          cmpl   $0x7,0x8(%rsp)	# x1<7
  400f6f:       77 3c                   ja     400fad <phase_3+0x6a>
  400f71:       8b 44 24 08             mov    0x8(%rsp),%eax
  400f75:       ff 24 c5 70 24 40 00    jmpq   *0x402470(,%rax,8)	# goto switch
  ...
  400fbe:       3b 44 24 0c             cmp    0xc(%rsp),%eax	# x2 = some switch val
...
# let temp==f1
0000000000400fce <func4>:	# f1:%edi f2:%esi f3:%edx
  400fce:       48 83 ec 08             sub    $0x8,%rsp
  400fd2:       89 d0                   mov    %edx,%eax	# ret=f3
  400fd4:       29 f0                   sub    %esi,%eax	# ret-=f2
  400fd6:       89 c1                   mov    %eax,%ecx	# temp=ret
  400fd8:       c1 e9 1f                shr    $0x1f,%ecx	# temp>>=31
  400fdb:       01 c8                   add    %ecx,%eax	# ret+=temp
  400fdd:       d1 f8                   sar    %eax			# ret>>=1
  400fdf:       8d 0c 30                lea    (%rax,%rsi,1),%ecx	# temp=ret+f2
  400fe2:       39 f9                   cmp    %edi,%ecx		
  400fe4:       7e 0c                   jle    400ff2 <func4+0x24>	# temp<=f1 return0
  400fe6:       8d 51 ff                lea    -0x1(%rcx),%edx	# f3--
  400fe9:       e8 e0 ff ff ff          callq  400fce <func4>	# recur
  400fee:       01 c0                   add    %eax,%eax	# ret*=2
  400ff0:       eb 15                   jmp    401007 <func4+0x39>	# ret
  400ff2:       b8 00 00 00 00          mov    $0x0,%eax	# ret=0
  400ff7:       39 f9                   cmp    %edi,%ecx	# temp>=f1, f1==temp
  400ff9:       7d 0c                   jge    401007 <func4+0x39>	# return 0
  400ffb:       8d 71 01                lea    0x1(%rcx),%esi	# f2=temp+1
  400ffe:       e8 cb ff ff ff          callq  400fce <func4>	# recur
...
000000000040100c <phase_4>:
  ...
  401010:       48 8d 4c 24 0c          lea    0xc(%rsp),%rcx	# x2
  401015:       48 8d 54 24 08          lea    0x8(%rsp),%rdx	# x1
  40101a:       be cf 25 40 00          mov    $0x4025cf,%esi
  40101f:       b8 00 00 00 00          mov    $0x0,%eax
  401024:       e8 c7 fb ff ff          callq  400bf0 <__isoc99_sscanf@plt>	# read 2 int
  401029:       83 f8 02                cmp    $0x2,%eax
  40102c:       75 07                   jne    401035 <phase_4+0x29>	# read finish
  40102e:       83 7c 24 08 0e          cmpl   $0xe,0x8(%rsp)	# x1<=14
  401033:       76 05                   jbe    40103a <phase_4+0x2e>
  401035:       e8 00 04 00 00          callq  40143a <explode_bomb>
  40103a:       ba 0e 00 00 00          mov    $0xe,%edx	# f3=14
  40103f:       be 00 00 00 00          mov    $0x0,%esi	# f2=0
  401044:       8b 7c 24 08             mov    0x8(%rsp),%edi	# f1=x1
  401048:       e8 81 ff ff ff          callq  400fce <func4>	# func4(f1,f2,f3)
  40104d:       85 c0                   test   %eax,%eax	# func4 return 0
  40104f:       75 07                   jne    401058 <phase_4+0x4c>
  401051:       83 7c 24 0c 00          cmpl   $0x0,0xc(%rsp)	# x2=0
  401056:       74 05                   je     40105d <phase_4+0x51>
  401058:       e8 dd 03 00 00          callq  40143a <explode_bomb>
  40105d:       48 83 c4 18             add    $0x18,%rsp
  401061:       c3                      retq

0000000000401062 <phase_5>:	# input: %rdi
  401062:       53                      push   %rbx
  401063:       48 83 ec 20             sub    $0x20,%rsp
  401067:       48 89 fb                mov    %rdi,%rbx
  40106a:       64 48 8b 04 25 28 00    mov    %fs:0x28,%rax	# stack protector
  401071:       00 00
  401073:       48 89 44 24 18          mov    %rax,0x18(%rsp)
  401078:       31 c0                   xor    %eax,%eax
  40107a:       e8 9c 02 00 00          callq  40131b <string_length>
  40107f:       83 f8 06                cmp    $0x6,%eax	# input.len=6
  401082:       74 4e                   je     4010d2 <phase_5+0x70>
  401084:       e8 b1 03 00 00          callq  40143a <explode_bomb>
  401089:       eb 47                   jmp    4010d2 <phase_5+0x70>
  40108b:       0f b6 0c 03             movzbl (%rbx,%rax,1),%ecx	# temp=input[%rax]
  40108f:       88 0c 24                mov    %cl,(%rsp)			
  401092:       48 8b 14 24             mov    (%rsp),%rdx			
  401096:       83 e2 0f                and    $0xf,%edx	# %rdx=char2int()[0:8]
  401099:       0f b6 92 b0 24 40 00    movzbl 0x4024b0(%rdx),%edx	# get char
  4010a0:       88 54 04 10             mov    %dl,0x10(%rsp,%rax,1)	# move char
  ...
  4010d2:       b8 00 00 00 00          mov    $0x0,%eax	# ret=0
  4010d7:       eb b2                   jmp    40108b <phase_5+0x29>
  4010d9:       48 8b 44 24 18          mov    0x18(%rsp),%rax
  4010de:       64 48 33 04 25 28 00    xor    %fs:0x28,%rax	# stack protector
  4010e5:       00 00
...
00000000004010f4 <phase_6>:
  ...
  401106:       e8 51 03 00 00          callq  40145c <read_six_numbers>	# read int
  40110b:       49 89 e6                mov    %rsp,%r14
  40110e:       41 bc 00 00 00 00       mov    $0x0,%r12d	# i=0
  401114:       4c 89 ed                mov    %r13,%rbp	# rbp=rsp	:sec2
  401117:       41 8b 45 00             mov    0x0(%r13),%eax	# %eax=x[0]
  40111b:       83 e8 01                sub    $0x1,%eax
  40111e:       83 f8 05                cmp    $0x5,%eax	# x[0]<=5
  401121:       76 05                   jbe    401128 <phase_6+0x34>
  401123:       e8 12 03 00 00          callq  40143a <explode_bomb>
  401128:       41 83 c4 01             add    $0x1,%r12d	# i++
  40112c:       41 83 fc 06             cmp    $0x6,%r12d	
  401130:       74 21                   je     401153 <phase_6+0x5f>	# i==6 jump
  401132:       44 89 e3                mov    %r12d,%ebx	# k=i
  401135:       48 63 c3                movslq %ebx,%rax	# :sec1
  401138:       8b 04 84                mov    (%rsp,%rax,4),%eax	# x[k]
  40113b:       39 45 00                cmp    %eax,0x0(%rbp)	# x[k]!=x[0]
  40113e:       75 05                   jne    401145 <phase_6+0x51>
  401140:       e8 f5 02 00 00          callq  40143a <explode_bomb>
  401145:       83 c3 01                add    $0x1,%ebx	# k++
  401148:       83 fb 05                cmp    $0x5,%ebx	# k<=5 jump: sec1
  40114b:       7e e8                   jle    401135 <phase_6+0x41>
  40114d:       49 83 c5 04             add    $0x4,%r13	# x+=4
  401151:       eb c1                   jmp    401114 <phase_6+0x20>	# jump: sec2
...
# sort of list
0000000000401204 <fun7>:	# f1=0x6030f0:%edi f2=x:%esi
  401204:       48 83 ec 08             sub    $0x8,%rsp
  401208:       48 85 ff                test   %rdi,%rdi	# f1!=NULL
  40120b:       74 2b                   je     401238 <fun7+0x34>
  40120d:       8b 17                   mov    (%rdi),%edx	# temp=*f1
  40120f:       39 f2                   cmp    %esi,%edx	
  401211:       7e 0d                   jle    401220 <fun7+0x1c>	# temp<f2 jump
  401213:       48 8b 7f 08             mov    0x8(%rdi),%rdi	# f1.next1
  401217:       e8 e8 ff ff ff          callq  401204 <fun7>
  40121c:       01 c0                   add    %eax,%eax	# ret*=2
  40121e:       eb 1d                   jmp    40123d <fun7+0x39>	# return
  401220:       b8 00 00 00 00          mov    $0x0,%eax	# ret=0
  401225:       39 f2                   cmp    %esi,%edx
  401227:       74 14                   je     40123d <fun7+0x39>	# temp==f2 return 0
  401229:       48 8b 7f 10             mov    0x10(%rdi),%rdi	# f1.next2
  40122d:       e8 d2 ff ff ff          callq  401204 <fun7>
  401232:       8d 44 00 01             lea    0x1(%rax,%rax,1),%eax	# return ret*2+1
...
0000000000401242 <secret_phase>:
  ...
  401252:       48 89 c7                mov    %rax,%rdi	# read line
  401255:       e8 76 f9 ff ff          callq  400bd0 <strtol@plt>	# str2int
  40125a:       48 89 c3                mov    %rax,%rbx
  40125d:       8d 40 ff                lea    -0x1(%rax),%eax	# temp=x-1
  401260:       3d e8 03 00 00          cmp    $0x3e8,%eax	# temp<=1000
  401265:       76 05                   jbe    40126c <secret_phase+0x2a>
  401267:       e8 ce 01 00 00          callq  40143a <explode_bomb>
  40126c:       89 de                   mov    %ebx,%esi	# f2=x
  40126e:       bf f0 30 60 00          mov    $0x6030f0,%edi	# f1=0x6030f0	
  401273:       e8 8c ff ff ff          callq  401204 <fun7>
  401278:       83 f8 02                cmp    $0x2,%eax	# ret==2
...
00000000004015c4 <phase_defused>:
  ...
  4015f5:       bf 70 38 60 00          mov    $0x603870,%edi	# input line 4
